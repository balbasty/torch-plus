...
def max_dtype(*args, force_float=False):
    ...
    *args : tensor_like or type_like
    ...
    is_tensor = torch.is_tensor
    def is_array(x): return isinstance(x, np.ndarray)
    def is_np_dtype(x): return isinstance(x, np.dtype) or (isinstance(x, type) and issubclass(x, np.number))
    def is_torch_dtype(x): return isinstance(x, torch.dtype)
    def is_py_dtype(x): return isinstance(x, type) and issubclass(x, numbers.Number)
    def is_dtype(x): return is_torch_dtype(x) or is_np_dtype(x) or is_py_dtype(x)
