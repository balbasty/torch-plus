...
_help_ij = _help.format(warnxy="")
_help_xy = _help.format(warnxy=_warnxy)
...
if torch_version('>=', (1, 10)):
    ...
    if not int(os.environ.get('PYTORCH_JIT', '1')):
        ...
        @torch.jit.script
        def meshgrid_list_ij(tensors: List[torch.Tensor]) -> List[torch.Tensor]:
            return list(torch.meshgrid(*tensors, indexing='ij'))

        @torch.jit.script
        def meshgrid_list_xy(tensors: List[torch.Tensor]) -> List[torch.Tensor]:
